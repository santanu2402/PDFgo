...
import { wordtopdf } from '../api/ApiCalls';
import NoInternetConnection from '../components/NoInternetConnection';

const screenWidth = Dimensions.get('window').width;
const screenHeight = Dimensions.get('window').height;

const WordToPdfScreen = (props: any) => {
  ...
  const convertToPdf = async () => {
    if (!fileUri) {
      Alert.alert('No File Selected', 'Please select a DOC/DOCX file first.');
      return;
    }
    ...
    try {
      ...
      const response = await wordtopdf(fileName, fileContent)
      ...
      fileReaderInstance.onload = async () => {
        ...
        const path = `/storage/emulated/0/Download/PDFgo/WordToPdf/${fileName}_converted.pdf`;
  ...
  return (
    <SafeAreaView style={{ height: screenHeight, width: screenWidth, backgroundColor: COLORS.lightBackground }}>
      ...
      <ScrollView contentContainerStyle={{ alignItems: 'center', padding: 16 }}>
        ...
        {pdfUri && (
          <>
            <Text style={{ color: COLORS.primary, fontFamily: FONTFAMILY.poppins_bold, fontSize: 22, textAlign: 'center' }}>
              Preview PDF
            </Text>
            <Pdf
              source={{ uri: pdfUri }}
